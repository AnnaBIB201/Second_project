    ...
    while (window.isOpen()) {
        ...
        if (Niko.Score >= 0) {
            if ((Keyboard::isKeyPressed(Keyboard::A)) &&
                ((LENGTH - Niko.x) <= LENGTH)) {
                Niko.MoveToLeft(Moment, times);
                Niko.sprite.setTextureRect(
                    IntRect(220 * int(Moment) + 220, 440, -220, 280));
                ChangeCadr(Moment);
            }
            if ((Keyboard::isKeyPressed(Keyboard::D)) &&
                (Niko.x <= (LENGTH - Niko.w))) {
                Niko.MoveToRight(Moment, times);
                Niko.sprite.setTextureRect(
                    IntRect(220 * int(Moment), 440, 220, 280));
                ChangeCadr(Moment);
            }
