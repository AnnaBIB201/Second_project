...
void gameOver(RenderWindow& window) {
	...
	while (theEnd) {
		...
		if (IntRect(1200, 600, 144, 64).contains(Mouse::getPosition(window))) {
			gameExit.setColor(Color::Red); endNum = 1;
		}
		if (Mouse::isButtonPressed(Mouse::Left)) {
			if (endNum == 1) { window.close(); theEnd = false; }
		}
		window.draw(end);
		window.draw(text);
...
void inMenu(RenderWindow& window) {
	...
	Texture menuTexture1, menuTexture2, menuTexture3, menuTexture4, menuTexture5, menuTexture6, menuBackground, menuRule;
	...
	Sprite menu(menuTexture1), start(menuTexture2), rules(menuTexture3), exit(menuTexture4), menuBg(menuBackground), rules2(menuTexture5), pict(menuTexture6), rule(menuRule);
	bool isMenu = 1;
	int menuNum = 0;
	...
	while (isMenu)
	{
		...
		if (Mouse::isButtonPressed(Mouse::Left))
		{
			...
			if (menuNum == 2) { window.draw(rules2); window.draw(rule); window.display(); while (!Keyboard::isKeyPressed(Keyboard::Escape)); }
